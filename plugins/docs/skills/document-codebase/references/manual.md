...
7. **Settle the six authored pages** (see below). Fill each scaffold `publish` wrote, or waive it with an
   owner and a reason. Publication is held until every one is `complete` or `waived`.
8. Review the rendered RST: does it read as a manual, and does each section still say what its answers said?
...
`publish` writes a scaffold for every unsettled page that has no file yet — **never overwriting one that
