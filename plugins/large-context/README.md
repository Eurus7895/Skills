...
- **`synthesize-corpus`** — for text corpora: logs, support tickets, contracts, transcripts, papers, exports.
  Probes the corpus for its own vocabulary, extracts every unit into a declared schema, validates coverage with
  `assemble.py`, and reports figures with source ids. Fires on "how many of these", "top 3 reasons", "find
  every", "audit these", "compare across".
- **`document-codebase`** — for repositories. Parses structure with `scan_repo.py` (Python via `ast`, other
  languages by import regex), ranks modules by fan-in, describes each one with its real callers supplied, then
  cross-checks every structural claim against the dependency graph before writing `docs/ARCHITECTURE.md`.
  Fires on "document this repo", "explain how this fits together", "map the dependencies".
...
- **Coverage is verified, not assumed.** `assemble.py` exits non-zero when units produced no row, when fields
  came back mostly empty, or when a field is suspiciously constant. Figures from a table that failed validation
  are not reported.
